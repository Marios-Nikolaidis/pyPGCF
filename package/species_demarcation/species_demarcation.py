...
def create_input_for_fastani(files_for_fastani: Union[List, Generator], tmp_file_for_fastani: Path) -> None:
    with open(str(tmp_file_for_fastani), "w") as f:
        for file in files_for_fastani:
            f.write(str(file) + "\n")

def perform_fastani(org_list: Path, threads: int, fraglen: int, min_fraction: float, kmer_size: int, fout: Path) -> None:
...
def run_mcl(fastani_for_mcl: Path, inflation: float, threads: int) -> Path:
...
def parse_mcx_output(fastani_from_mcl: Path) -> None:
